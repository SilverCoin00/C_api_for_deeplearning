...
void Adam_1D(Weights* w, Weights* grad, float lr, Weights* acc_grad, 
                float beta_1, float beta_2, float ep, Weights* pre_velo, float b1, float b2) {
    float velo, accg;
    int row = w->w->row, col = w->w->col, i, j;
    for (i = 0; i < col; i++) {
        for (j = 0; j < row; j++) {
...
void Adam_3D(Kernel* w, Kernel* grad, float lr, Kernel* acc_grad, 
                float beta_1, float beta_2, float ep, Kernel* pre_velo, float b1, float b2) {
    float velo, accg;
    int channel = w->channel, depth =  w->w[0]->depth, row = w->w[0]->mat[0]->row, col = w->w[0]->mat[0]->col, i, j, k, h;
    for (i = 0; i < channel; i++) {
        for (j = 0; j < depth; j++) 
...
void adaptive_moment_estimation(int type, void* layer, float learning_rate, float beta_1, float beta_2, float epsilon, int* is_new) {
    ...
    if (type == 1) {
        Dense* l = (Dense*) layer;
        Adam_1D(l->weights, l->deriv, learning_rate, l->acc_grad, beta_1, beta_2, epsilon, l->pre_velo, b1, b2);
    } else if (type == 3) {
        Conv* l = (Conv*) layer;
        Adam_3D(l->filter, l->deriv, learning_rate, l->acc_grad, beta_1, beta_2, epsilon, l->pre_velo, b1, b2);
    }
}